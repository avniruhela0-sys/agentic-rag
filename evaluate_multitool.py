"""
Day 24-25: Full Evaluation with Multi-Tool Agent
Comprehensive testing of RAG + Web Search agent.
"""
...
print("=" * 60)
print("🧪 DAY 24-25: MULTI-TOOL AGENT EVALUATION")
print("=" * 60)
...
def should_route(state: AgentState) -> str:
    ...
    web_search_used = state.get("web_search_used_in_loop", False)
    
    print(f"\n   🧠 ROUTING DECISION:")
    print(f"      Score: {score}/10")
    print(f"      Attempts: {attempts}/2")
    print(f"      Web Search Already Used: {web_search_used}")
    
    if score >= 7.0:
        print(f"      ✅ Score >= 7 → FINALIZE")
        return "finalize"
    if attempts >= 2:
        print(f"      ⚠️ Max attempts → FINALIZE")
        return "finalize"
    if web_search_used:
        print(f"      🔄 Web search used → CORRECT")
        return "correct"
    
    # Check if question needs web search
    question_lower = state["question"].lower()
    current_keywords = ['current', 'latest', 'recent', '2024', '2025', '2026', 'ceo', 'president']
    if any(kw in question_lower for kw in current_keywords):
        print(f"      🌐 Current topic → WEB_SEARCH")
        return "web_search"
    
    if len(state.get("retrieved_docs", [])) < 3 and attempts >= 1:
        print(f"      🌐 Poor retrieval → WEB_SEARCH")
        return "web_search"
...
# ============================================
# Part 8: Test Questions
# ============================================
print("\n📋 PART 8: TEST QUESTIONS")
print("-" * 40)

# Mix of factual and current questions
test_questions = [
...
for i, question in enumerate(test_questions, 1):
    ...
    try:
        ...
        agent_correct = is_correct("", agent_answer)  # No expected answer for these
    except Exception as e:
    ...
    print(f"\n   📊 Results:")
    print(f"      Agentic Score: {agent_score:.1f}/10")
    print(f"      Attempts: {agent_attempts}")
    print(f"      Web Search: {'✅' if used_web else '❌'}")
...
web_used_count = sum(1 for r in agent_results if r['used_web_search'])
...
│ Web Search Used                     │ {web_used_count}/{len(test_questions)}         │
...
results = {
    ...
    "web_search_used": web_used_count,
    "avg_time": avg_time,
...
with open('multitool_eval_results.json', 'w') as f:
    json.dump(results, f, indent=2)

print("✅ Saved results to multitool_eval_results.json")
...
print("-" * 40)
print(f"""
✅ Tested multi-tool agent on {len(test_questions)} questions
✅ Average Judge Score: {avg_score:.1f}/10
✅ Average Correction Attempts: {avg_attempts:.1f}
✅ Web Search Used: {web_used_count} times
✅ Average Time: {avg_time:.2f}s

📁 FILES CREATED:
   - multitool_eval_results.json

🎯 Week 4 Complete! You've built:
   1. Web Search (Tavily) ✅
   2. Intelligent Routing ✅
   3. Multi-Tool Agent ✅
   4. Full Evaluation ✅

🚀 Next: Week 5 - Stretch Goals (UI, Demo, Final Polish)
""")

print("\n🚀 Ready for Week 5! (Stretch Goals)")
print("=" * 60)